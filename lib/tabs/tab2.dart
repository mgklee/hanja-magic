...
class _Tab2State extends State<Tab2> {
  ...
  Future<void> _addApp(String appName) async {
    if (!installedAppNames.any((name) => name.toLowerCase() == appName.toLowerCase())) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('App not found: $appName')));
      return;
    }
    try {
      final info = await platform.invokeMethod('getSingleAppInfoByName', {'appName': appName});
      final String name = info['name'] ?? '';
      final String packageName = info['package'] ?? '';
      final String iconBase64 = info['icon'] ?? '';
      setState(() {
        _apps.add({'name': name, 'package': packageName, 'icon': iconBase64});
      });
      await _saveApps();
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('App "$name" added!')));
    } on PlatformException catch (_) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to add $appName')));
    }
  }
  ...
  Future<void> _launchApp(String packageName) async {
    try {
      ...
      if (!success) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Cannot launch $packageName')));
      }
    } on PlatformException catch (_) {}
  }
  ...
  Future<void> _deleteApp(String appName) async {
    ...
    await _saveApps();
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$appName 삭제되었습니다.')));
  }
                ...
                for (final app in _apps)
                  ListTile(
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () => _showDeleteDialog(app['name'] ?? ''),
                    ),
